 ...
 *
 * Vikas : the sequence of calls are as follows.
 * One has to load the bcm module in the kernel
 * DYNCHS = Dynamic channel setup
 * The server has to be started b4 the client other wise it gives an error of unable to get channel set up reply
 * If the tpgen- module is not loaded then the client recieves a correct channel set up reply but a socket create error
 * of tp20.. 
 * OUTPUT
 * Received correct channel setup reply.
 *
 * Wait for connection on vcan2: tx_id=0x242 rx_id=0x321 ... connected!
 *
 * Sending message 'einnegermitgazellezagtimregennie' ... done!
 *
 * Waiting for reply message ... ok.
 * Received 32 bytes: 'EINNEGERMITGAZELLEZAGTIMREGENNIE'
 *
 * Closing connection.
 */
...
#define REMOTE_TP_ID 0x5d
#define LOCAL_TP_RX_ID 0x4ec
#define LOCAL_TP_TX_ID 0x300
