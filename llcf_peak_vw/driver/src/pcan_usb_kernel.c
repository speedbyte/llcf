...
//
// $Id: pcan_usb_kernel.c 451 2007-02-02 10:09:50Z ohartkopp $
//
//****************************************************************************
...
int pcan_hw_DecodeMessage(struct pcandev *dev, u8 *ucMsgPtr, int lCurrentLength)
{
  ...
  for (i = 0; (i < ucMsgLen); i++)
  {
    ...
    if (!(ucStatusLen & STLN_INTERNAL_DATA))
    {
      ...
      {
        frame.can_id |= MSGTYPE_EXTENDED; // modify if it was extended
        ...
        localID.uc[1] = *ucMsgPtr++;
        localID.uc[0] = *ucMsgPtr++;
        #warning "Compiling for __BIG_ENDIAN"
        #endif
      ...
      #ifdef NETDEV_SUPPORT
      if ((err = pcan_netdev_rx(dev, &frame, &tv))) /* put to network layer queue */
        goto fail; 
      #else
      if ((err = pcan_chardev_rx(dev, &frame, &tv))) /* put to chardev FIFO */
        goto fail; 
      rwakeup++;
      #endif
    ...
    else
    {
      ...
      if (ef.can_id) 
      {
        ...
        #ifdef NETDEV_SUPPORT
        if ((err = pcan_netdev_rx(dev, &ef, &tv))) /* put to network layer queue */
          goto fail;
        #else
        if ((err = pcan_chardev_rx(dev, &ef, &tv))) /* put to chardev FIFO */
          goto fail;
        rwakeup++;
        #endif
      } 
