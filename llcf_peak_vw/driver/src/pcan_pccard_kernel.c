...
//
// $Id: pcan_pccard_kernel.c 447 2007-01-28 14:05:50Z khitschler $
//
//****************************************************************************
...
#define CCR_DEFAULT       ((((u8)CCR_LED_OFF) << CCR_LED0_SHIFT) | (((u8)CCR_LED_OFF) << CCR_LED1_SHIFT)) 
...
// helper macros
#ifdef XENOMAI
#define pcan_mutex_init(x)   rtdm_mutex_init(x)
#define pcan_mutex_lock(x)   rtdm_mutex_lock(x)
#define pcan_mutex_unlock(x) rtdm_mutex_unlock(x)
#else
#define pcan_mutex_init(x)   spin_lock_init(x)
#define pcan_mutex_lock(x)   spin_lock(x)
#define pcan_mutex_unlock(x) spin_unlock(x)
#endif
...
{
  pcan_mutex_init(&card->lock);
  pcan_mutex_lock(&card->lock);
  pccard_writereg_common(card, CCR, CCR_DEFAULT);  
  pcan_mutex_unlock(&card->lock);
}
...
static irqreturn_t IRQHANDLER(pcan_pccard_irqhandler, int irq, void *dev_id, struct pt_regs *regs)
{
  ...
  {
    if ((dev = card->dev[index % PCCARD_CHANNELS])) // consider singel channel cards
    {
...
static int pccard_cleanup(struct pcandev *dev)
{
  ...
  if (dev)
  {
    ...
    switch(dev->wInitStep)
    {
      ...
      case 4: if (card) 
                pccard_set_LED(card, dev->port.pccard.nChannel, CCR_LED_OFF); 
              #ifdef NETDEV_SUPPORT
